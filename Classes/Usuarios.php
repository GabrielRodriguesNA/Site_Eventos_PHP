...
class Usuarios
{
    ...
    public function save()
    {
        ...
        $existingUser = Usuarios::getUserByEmail($this->email);
    ...
    public static function authenticate($email, $password)
    {
        ...
        if ($user && password_verify($password, $user['password'])) {
            return new Usuarios($user['id'], $user['name'], $user['email'], $user['password'], $user['user_type']);
        } else {
            return null;
        }
    }
    ...
    public static function getUserByEmail($email)
    {
        ...
        $stmt->close();
        $conn->close();
        return $user ? new Usuarios($user['id'], $user['name'], $user['email'], $user['password'], $user['user_type']) : null;
    }
    ...
    public static function getUsers($search = null) {
        ...
        while ($row = $result->fetch_assoc()) {
            $user = new Usuarios($row['id'], $row['name'], $row['email'], $row['password'], $row['user_type']);
            $users[] = $user;
        }
        $stmt->close();
        $conn->close();
